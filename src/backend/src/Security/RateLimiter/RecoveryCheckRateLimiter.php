...
class RecoveryCheckRateLimiter implements RecoveryCheckRateLimiterInterface
{
    ...
    private function getLimiters(UserInterface $user): LimiterInterface
    {
        ...
        $limit = $securitySetting->getMfaMaxResend();
        $interval = $securitySetting->getLoginInterval();

        $attemptLimiterFactory = new RateLimiterFactory(
            [
                'id' => MFAConfig::MFA_LIMITER.MFAConfig::GOOGLE_AUTHENTICATION,
                'policy' => 'fixed_window',
                'limit' => $limit,
